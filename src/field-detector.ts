...
function castRay(
  ...
  // Rolling average seed adapts to gradual color changes (shadows, wear)
  ...
  for (let d = step; d < maxDist; d += step) {
    ...
    const minColorDist = Math.min(distFromSeed, distFromRolling);

    if (minColorDist > tolerance) break;

    dist = d;
    endX = px;
    endY = py;
    // Slowly adapt rolling seed (90% old, 10% new)
    rr = rr * 0.9 + pr * 0.1;
    rg = rg * 0.9 + pg * 0.1;
    rb = rb * 0.9 + pb * 0.1;
  }
